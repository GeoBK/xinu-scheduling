...
shellcmd xsh_ps(int nargs, char *args[])
{
	...
	printf("%3s %-16s %5s %4s %4s %10s %-10s %10s %10s %10s %10s\n",
		   "Pid", "Name", "State", "Prio", "Ppid", "Stack Base",
		   "Stack Ptr", "Stack Size", "Turnaround", "#ctxsw", "runtime");
	...
	for (i = 0; i < NPROC; i++) {
		...
		printf("%3d %-16s %s %4d %4d 0x%08X 0x%08X %8d %10d %10d %10d\n",
			i, prptr->prname, pstate[(int)prptr->prstate],
			prptr->prprio, prptr->prparent, prptr->prstkbase,
			prptr->prstkptr, prptr->prstklen, prptr->runtime, prptr->turnaroundtime, prptr->num_ctxsw);
	}
	return 0;
}
